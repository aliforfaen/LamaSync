...
export interface ResolvedClient {
  ...
  /**
   * True when neither env vars nor a parseable config file provided
   * credentials. Commands treat this as fatal for mutations (refuse fast,
   * LAMA-248) and merely informational for diagnostics (status / doctor).
   * A config file that exists but fails to parse is ALSO a setup
   * situation.
   */
  needsSetup: boolean;
  error?: string;
...
export function buildClient(): ResolvedClient {
