 ...
 *
 * Local-first surface (LAMA-326): the CLI keeps only what talks to the
 * local daemon (`local.*` over the Unix socket), host diagnostics
 * (`doctor`), and device pairing (`register`, LAMA-262). Full-fleet
 * management lives in the web UI and the REST API
 * (`packages/agent-skill/reference/api.md`); the former LAMA-248
 * no-config refusal is gone with the server-facing command surface.
 */
...
import { CliFailError, maskSecret } from "./output.ts";
...
const TOP_HELP = {
  summary:
    "lamasync — local control for the LamaSync daemon on this device. Bare " +
    "`lamasync` prints this help; any subcommand exits non-interactively.",
...
function doctorHelp(): { summary: string; usage: string } {
  return {
    ...
    usage:
      ...
      `  Runs without a client.toml — diagnosing the missing-config case is\n` +
      `  part of the job. Fleet management lives in the web UI / REST API.`,
  };
}
...
export async function runCli(argv: string[]): Promise<void> {
  try {
    ...
    // Decisive command failure (fail()): the code was chosen at the
    // callsite — write the line and exit; don't re-map through
    // exitCodeForError.
    if (err instanceof CliFailError) {
