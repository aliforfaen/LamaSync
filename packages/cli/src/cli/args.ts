 ...
 *
 * The CLI is intentionally zero-dependency (matches the rest of the
 * server/daemon), so we walk argv ourselves. The shape we return
 ...
 *
 * Why `hasCommand`? The binary entry (`src/index.ts`) must decide
 * whether argv contains a subcommand (CLI dispatch) or just bare flag
 * overrides. A naive "first non-flag token is the
...
export interface ParsedArgs {
   ...
   *  bare flags). The binary entry uses this to route to the CLI
   *  dispatcher instead of printing top-level help. */
  hasCommand: boolean;
}
