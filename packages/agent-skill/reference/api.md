...
| POST     | `/mobile/enrollments`                     | (admin) create a mobile (Android companion) enrollment — `{ webAdmin, clientType? }` → `{ enrollmentId, secret, serverOrigin, clientType, webAdmin, expiresAt, expiresInSeconds }`. `secret` is one-time (QR); creating a new enrollment revokes other still-pending ones (LAMA-296). Requires `LAMASYNC_ORIGIN` |
...
| POST     | `/mobile/registrations/:hostId/revoke`    | (admin) atomically revoke the native credential, web grant and all web sessions for a registration, mark its enrollment revoked, disconnect live WebSockets. Idempotent (LAMA-296) |
