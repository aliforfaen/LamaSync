...
- LAMA-345 managed-folder health contract (shared by daemon, server and web UI, exported from `@lamasync/core/folder-health`):
  ...
  - `FolderSyncPlan { id, hostId, folderId, assignmentId, intervention, authority, summary, changes, configRevision, filterFingerprint, baselineFingerprint, createdAt, expiresAt }` — a reviewed dry-run intent. `intervention` is `initialize | seed | resync`; `authority` is `remote | local`. With the daemon's `bisync remote local` command shape, the remote is **Path 1** and the local tree is **Path 2**, so `remote` means `--resync-mode path1` (the remote wins and local-only files are removed) and `local` means `--resync-mode path2` (this device wins and remote-only files are removed there). `changes` is capped at 20 entries per list.
  ...
  - `FolderInterventionPayload { folderId, intervention, authority?, planId?, confirm?, maxDelete? }` — the complete allowlist for a mutating folder action. `initialize` requires `authority: "remote"`, `seed` requires `authority: "local"`, `resync` requires an explicit `authority` of either side; all three require a reviewed `planId` and `confirm: true`. `resume` and `cancel` take neither authority nor plan. Nothing else is accepted.
...
- LAMA-345 (folder health + intervention). Read `GET /folders/:id/health` for `records[hostId]`: `state` is one of `healthy | new_host | resync_required | recoverable | unsafe | blocked | busy | unknown`, and `reasons[0]` carries the message plus the exact remediation. The workflow is **Diagnose → Plan → Approve → Execute → Verify**:
  ...
  2. `POST /hosts/:hostId/actions {"type":"plan_folder","payload":{"folderId":"<id>","intervention":"initialize|seed|resync","authority":"remote|local"}}` runs a real dry run against the device's own bisync workdir. Wait, then read `GET /folders/:id/plans` and take `plan.id` — `validity.valid` must be `true`, and anything else tells you to plan again.
  3. Approve with `POST /hosts/:hostId/actions {"type":"folder_intervention","payload":{"folderId":"<id>","intervention":"…","authority":"…","planId":"<plan.id>","confirm":true}}`. `initialize` only accepts remote authority, `seed` only local authority. The daemon re-checks the plan against its own config revision, filter fingerprint and listing pair and refuses a stale one.
  4. `intervention: "resume"` continues recoverable work with no resync; `intervention: "cancel"` stops an in-flight run and reports the distinct `cancelled` status (never `failed`).
