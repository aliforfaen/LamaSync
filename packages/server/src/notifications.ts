...
async function postJson(
  ...
  targetName: "ntfy" | "LamaDB webhook",
): Promise<boolean> {
  try {
...
function scheduleDeliveries(event: NotificationEvent, database: Database): void {
  ...
  if (ntfyUrl && event.severity !== "info") {
    const topic = topicFromUrl(ntfyUrl);
    if (topic) {
      void postJson(
        ntfyUrl,
        {
        ...
        if (delivered) markDelivered(database, event.id, "ntfy_delivered");
      });
    }
  }
  ...
  if (webhookUrl) {
    void postJson(
      ...
      if (delivered) markDelivered(database, event.id, "webhook_delivered");
    });
  }
}
