...
placement, minimum viewport floor. Post proposal to LAMA-275 comments; **do not
silently decide**. Reversible work only until approved.

### Phase 3 — LAMA-275: implementation (after approval)
...
Before/after artifacts at all supported sizes.

### Phase 4 — copy passes against the glossary (LAMA-251, LAMA-253)
Web-ui view copy first, then CLI/TUI help + usage blocks + wizard text +
error messages. Strings only; commands/flags/exit codes/JSON keys unchanged.
Re-run drift check after each.
