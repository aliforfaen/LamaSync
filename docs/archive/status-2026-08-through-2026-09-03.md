...
## LAMA-316 — app templates/protections/snapshots (2026-09-03)

**What landed**
- The dotfile/profile model is replaced by an explicit app contract:
  **ApplicationTemplate** (curated/custom capture recipe) → enrolled on
  exactly one host as an **ApplicationProtection** → scheduled/manual
  captures become immutable **ApplicationSnapshot** archives. New
  `application_templates` / `application_protections` /
  `application_snapshots` tables (schema + idempotent migration,
  `convertLegacyAppConfig`) lift legacy `app_profiles` and
  `dotfile_manifests` rows (host-specific + `_global` fan-out) into
  templates/protections. The legacy tables stay on disk read-only/unused —
  nothing is dropped.
- `/api/v1/apps` route family with device↔own-host boundaries: templates CRUD
  and all mutations are admin-only; device keys may only list/get their own
  host's protections and upload/download its snapshots. Tarballs land under
  `<LAMASYNC_BACKUP_DIR>/apps/<protectionId>/<timestamp>-<uuid>.tar.gz`; every
  snapshot records only the source host's OS capture bucket, with deterministic
  portable archive-member mappings, plus a sha256 checksum; disabled
  protections reject uploads with 409.
- Daemon capture rewired: `HostConfig.apps` (`AppCaptureAssignment[]`) is
  derived from enabled protections, retains logical home-relative paths beside
  their daemon-local expansion, and schedules captures directly without a
  dotfile folder assignment.
- Web UI (App templates + App backups pages) and the `lamasync apps` CLI
  (templates/protections/snapshots) replace the old Presets/Dotfiles surface
  and the `lamasync dotfiles` namespace.
- **Intentional break:** `_global` scoping and the `lamasync dotfiles` CLI
  namespace are removed WITHOUT aliases.
...
- **LAMA-247 no-issue batch landed on the branch**: S3 download streaming
  ...
  renderer smoke tests, `LAMASYNC_SOCKET_PATH` env table) still open in
  `docs/cleanup-2026-08-18.md`. Dotfile diff preview shipped in LAMA-288.
